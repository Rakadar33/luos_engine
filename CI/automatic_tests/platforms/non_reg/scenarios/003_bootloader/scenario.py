...
def run_scenario(tested_version= "main", upload = "ON"):
    ...
    for gate_position in range (network_node_number):
        ...
        platform.mcu.powerDown_Node(2)
        platform.mcu.powerDown_Node(3)
        platform.mcu.powerDown_Node(4)
        time.sleep(2)
        platform.mcu.powerUp_Node(4)
        platform.mcu.powerUp_Node(3)
        time.sleep(2)
        platform.mcu.powerUp_Node(2)
        time.sleep(2)
        ...
        ci_log.step_log(f"Flash App 1 ({firmPath['firmware_2']}) with bootloader", "Step")
        #assert(run_command(flash_node_A, timeout=40, verbose=True) != "ERROR")
        run_command(flash_node_A, timeout=40, verbose=True)
        ci_log.step_log(f"Flash App 2 ({firmPath['firmware_3']}) with bootloader", "Step")
        #assert(run_command(flash_node_B, timeout=40, verbose=True) != "ERROR")
        run_command(flash_node_A, timeout=40, verbose=True)
        ci_log.step_log(f"Flash App 4 ({firmPath['firmware_4']}) with bootloader", "Step")
