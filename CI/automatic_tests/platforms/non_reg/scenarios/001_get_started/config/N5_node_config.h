...
#define HAL_Platform_Init()                                 \
    ...
    GPIO_InitStruct_PTP_C.Pin              = PTP_D;       \
    GPIO_InitStruct_PTP_C.Mode             = GPIO_MODE_INPUT; \
    GPIO_InitStruct_PTP_C.Pull             = GPIO_PULLDOWN;       \
    /*GPIO_InitStruct_PTP_C.Speed            = GPIO_SPEED_FREQ_LOW;*/ \
    HAL_GPIO_Init(PTP_D_PORT, &GPIO_InitStruct_PTP_C);        \
...
#define HAL_Platform_Init()                                 \
    ...
    /*uint32_t init_timer;*/\
    /*init_timer = Luos_GetSystick();*/\
    /*while (Luos_GetSystick() - init_timer < 6);*/\
    /*HAL_GPIO_WritePin(PTP_D_PORT, PTP_D, GPIO_PIN_RESET);*/\
    /*while (Luos_GetSystick() - init_timer < 3);*/\
