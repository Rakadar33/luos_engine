...
class HubControl:
    ...
    # Private methods for classic HUB
    ...
    def _enable_default(self, position):
        if (position!= 0) and (position<= 10):
            print("Power ON MCU on port {position}")
            location, port = self._get_location(position)
            ...
            self.shell.run_command(start_command)

    def _disable_default(self, position):
        if (position!= 0) and (position<= 10):
            print("Power OFF MCU on HUB port {position}")
            location, port = self._get_location(position)            
            stop_command = f"echo 0 > sudo tee /sys/bus/usb/devices/{location}.{port}/authorized;"        
            stop_command += f"sudo uhubctl -a off -r 10 -l {location} -p {port};"        
            self.shell.run_command(stop_command)


    # Private methods for capable robot
